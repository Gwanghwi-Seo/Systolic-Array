...
module systolic_pe_array (
    ...
    input [`PE_ROW-1:0]                         PEARR_MAT_A_VALID_I,
    input [`DATA_WIDTH*`PE_ROW-1:0]             PEARR_MAT_A_DATA_I,
    
    // matrix B (Inputs for the first row)
    input [`PE_ROW_ID_WIDTH*`PE_COL-1:0]        PEARR_MAT_B_PE_ROW_ID_I,
    input [`PE_COL-1:0]                         PEARR_MAT_B_VALID_I,
    input [`DATA_WIDTH*`PE_COL-1:0]             PEARR_MAT_B_DATA_I,

    // PSUM (Inputs for the first row)
    input [`PE_COL-1:0]                         PEARR_MAT_PSUM_VALID_I,
    input [`ADDR_WIDTH*`PE_COL-1:0]             PEARR_MAT_PSUM_ADDR_I,
    input [`PSUM_WIDTH*`PE_COL-1:0]             PEARR_MAT_PSUM_DATA_I,

    // PSUM (Outputs from the last row)
    output [`PE_COL-1:0]                        PEARR_MAT_PSUM_VALID_O,
    output [`ADDR_WIDTH*`PE_COL-1:0]            PEARR_MAT_PSUM_ADDR_O,
    output [`PSUM_WIDTH*`PE_COL-1:0]            PEARR_MAT_PSUM_DATA_O
);
    ...
    generate
        ...
        for (m = 0; m < `PE_ROW; m = m + 1) begin
            assign mat_a_data[m][0]  = PEARR_MAT_A_DATA_I[`DATA_WIDTH*m +: `DATA_WIDTH];
            assign mat_a_valid[m][0] = PEARR_MAT_A_VALID_I[m];
        end
        for (n = 0; n < `PE_COL; n = n + 1) begin
            assign mat_b_data[0][n]   = PEARR_MAT_B_DATA_I[`DATA_WIDTH*n +: `DATA_WIDTH];
            assign mat_b_valid[0][n]  = PEARR_MAT_B_VALID_I[n];
            assign mat_b_pe_row_id[0][n] = PEARR_MAT_B_PE_ROW_ID_I[`PE_ROW_ID_WIDTH*n +: `PE_ROW_ID_WIDTH];
            
            assign mat_psum_data[0][n]  = PEARR_MAT_PSUM_DATA_I[`PSUM_WIDTH*n +: `PSUM_WIDTH];
            assign mat_psum_valid[0][n] = PEARR_MAT_PSUM_VALID_I[n];
            assign mat_psum_addr[0][n]  = PEARR_MAT_PSUM_ADDR_I[`ADDR_WIDTH*n +: `ADDR_WIDTH];
        end
        ...
        for (m = 0; m < `PE_ROW; m = m + 1) begin: G_PE_ROW
            for (n = 0; n < `PE_COL; n = n + 1) begin: G_PE_COL
                systolic_pe U_PE(
                    ...
                    .MAT_A_DATA_O       (mat_a_data[m][n+1]),
                    
                    // Matrix B connections (from top)
                    .PE_ROW_ID_I        (m[$clog2(`PE_ROW)-1:0]), // PE's own row ID
                    ...
                    .MAT_PSUM_ADDR_O    (mat_psum_addr[m+1][n]),
        ...
        for (n = 0; n < `PE_COL; n = n + 1) begin
            assign PEARR_MAT_PSUM_VALID_O[n] = mat_psum_valid[`PE_ROW][n];
            assign PEARR_MAT_PSUM_ADDR_O[`ADDR_WIDTH*n +: `ADDR_WIDTH] = mat_psum_addr[`PE_ROW][n];
