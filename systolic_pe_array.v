...
module systolic_pe_array (
    ...
    generate
        ...
        for (m = 0; m < `PE_ROW; m = m + 1) begin
            assign mat_a_data[m][0]  = MAT_A_I[`DATA_WIDTH*m +: `DATA_WIDTH];
            assign mat_a_valid[m][0] = MAT_A_VALID_I[m];
        end
        for (n = 0; n < `PE_COL; n = n + 1) begin
            ...
            assign mat_psum_addr[0][n]  = MAT_PSUM_ADDR_I[`ADDR_WIDTH*n +: `ADDR_WIDTH];
        end
        ...
        for (m = 0; m < `PE_ROW; m = m + 1) begin: G_PE_ROW
            for (n = 0; n < `PE_COL; n = n + 1) begin: G_PE_COL
                systolic_pe U_PE(
                    ...
                    .EN_PE_ROW_ID_I(en_pe_row_id[m][n]),
                    .MAT_B_VALID_I (mat_b_valid[m][n]),
                    .MAT_B_I       (mat_b_data[m][n]),
                    // Matrix B outputs (to bottom)
                    .EN_PE_ROW_ID_O(en_pe_row_id[m+1][n]),
